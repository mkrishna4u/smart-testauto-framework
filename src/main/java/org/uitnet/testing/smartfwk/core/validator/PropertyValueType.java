...
public enum PropertyValueType {
	...
	private PropertyValueType(String type) {
		this.type = type;
	}
	...
	public static PropertyValueType valueOf2(String type) {
		for (PropertyValueType t : values()) {
			if (t.getType().equals(type.toLowerCase()) || t.name().equalsIgnoreCase(type)) {
				return t;
			}
		}
		Assert.fail("Property value type '" + type + "' is not supported.");
		return null;
	}
