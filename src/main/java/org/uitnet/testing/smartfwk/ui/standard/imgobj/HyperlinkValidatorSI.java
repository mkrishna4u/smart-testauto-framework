...
public class HyperlinkValidatorSI extends HyperlinkValidator {
	...
	@Override
	public HyperlinkValidatorSI scrollElementOnViewport(ScrollbarSI scrollbar) {
	...
	@Override
	public HyperlinkValidatorSI validateElementPresentWithinArea(AreaCoordinates coordinates,
			int maxIterationsToLocateElements) {
		Match match = findElement(maxIterationsToLocateElements);
		if(!(match.x >= coordinates.getX1() && match.y >= coordinates.getY1() &&  
			...
			Assert.fail("Element '" + hyperlinkObj.getDisplayName() + "' is not within the specified area [x1=" + coordinates.getX1() 
