...
public class ImageValidatorSI extends ImageValidator {
	...
	@Override
	public ImageValidatorSI scrollElementOnViewport(ScrollbarSI scrollbar) {
	...
	public List<Match> findElements(int maxIterationsToLocateElements) {
		...
		return new ImageObject(UIObjectType.image, imageObj.getDisplayName(),
				imageObj.getImage(appDriver.getAppConfig().getTestPlatformType(), appDriver.getAppConfig().getAppType(),
						appDriver.getAppConfig().getAppWebBrowser()))
	...
	@Override
	public ImageValidatorSI validateElementPresentWithinArea(AreaCoordinates coordinates,
		...
		if(!(match.x >= coordinates.getX1() && match.y >= coordinates.getY1() &&  
			...
			Assert.fail("Element '" + imageObj.getDisplayName() + "' is not within the specified area [x1=" + coordinates.getX1() 
